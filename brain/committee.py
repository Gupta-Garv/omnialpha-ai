...
class MultiAgentCommittee:
    ...
    def evaluate_opportunity(self, symbol: str, account_summary: Dict[str, Any]) -> Dict[str, Any]:
        ...
        confidence_penalty = 0.05 if any("LESSON:" in l for l in past_lessons) else 0.0

        # 2. Decision Logic
        if sentiment == "BULLISH_VELOCITY" or (has_insider_activity and sentiment != "BEARISH_VELOCITY"):
            action = "PROPOSE_TRADE"
            strategy_type = "BULL_PUT_SPREAD"
            confidence = (0.85 if has_insider_activity else 0.75) - confidence_penalty
        elif sentiment == "BEARISH_VELOCITY":
            action = "PROPOSE_TRADE"
            strategy_type = "BEAR_CALL_SPREAD"
            confidence = 0.78 - confidence_penalty
        ...
        if action == "PROPOSE_TRADE":
            ...
            return {
                ...
                "audit_trail": {
                    ...
                    "rationale": f"High probability {strategy_type} based on {sentiment} velocity (Reflexion Lessons Applied: {len(past_lessons)})."
                },
                "reason": f"Signals aligned ({sentiment}, Insiders: {has_insider_activity}). Passed Risk Shield & Memory Check."
            }
